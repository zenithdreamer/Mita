    ...
    namespace transports
    {
        ...
        public:
            ...
            bool start_scan() override
            {
                try
                {
                    ...
                    // Try to start scan with a shorter timeout to avoid D-Bus timeout??
                    adapter_.scan_start();
                    scanning_ = true;

                    // Shorter initialization time to avoid blocking
                ...
                catch (const std::exception &e)
                {
                    ...
                    // InProgress is expected if already scanning - not an error
                    ...
                    // D-Bus timeout might be recoverable, i hope :(
            ...
            std::vector<BLEDeviceInfo> list_devices() override
            {
                ...
                try
                {
                    ...
                    // Start scanning only if not already active
                    if (!scanning_)
                    {
                        if (logger_)
                        ...
                        // Some delay to discovers devices
                    ...
                    {
                        // Give more time for fresh results when already scanning
                        std::this_thread::sleep_for(std::chrono::milliseconds(1500));
                    }

                    // Get current scan results without restarting scan
                    ...
                    for (auto &p : results)
                    {
                        // Use identifier as name if available, otherwise use address
                        std::string name = p.identifier().empty() ? p.address() : p.identifier();
                        BLEDeviceInfo info{p.address(), name};
                        out.push_back(info);
                        // Cache latest peripheral instance for quick lookup
                    ...
                    // Keep scanning active for continuous discovery
                }
                catch (const std::exception &e)
                {
                    if (logger_)
                        logger_->error("BLE scan failed", core::LogContext{}.add("error", e.what()));
                    // Don't reset scanning flag on InProgress errors - that's expected
            ...
            bool connect(const std::string &address) override
            {
                ...
                try
                {
                    if (!dev->is_connected())
                        dev->connect();
                    // SimpleBLE resolves services lazily; calling services() forces discovery
                    (void)dev->services();
                    peripherals_[address] = *dev; // store connected state
                    return true;
                }
                catch (const std::exception &e)
                {
                                      ...
                                      std::function<void(const std::string &, const std::vector<uint8_t> &)> cb) override
            {
                ...
                try
                {
                    ...
                    // Check if device is connected before setting up notifications
                    if (!dev->is_connected())
                    {
                        if (logger_)
                    ...
                    peripherals_[address] = *dev; // store with active notifications
        ...
        private:
            std::optional<SimpleBLE::Peripheral> get_or_refresh_peripheral(const std::string &address)
            {
                ...
                if (it != peripherals_.end())
                    return it->second;
                // Try to refresh from current scan results
                if (!adapter_.initialized())
                    return std::nullopt;
                auto results = adapter_.scan_get_results();
                for (auto &p : results)
