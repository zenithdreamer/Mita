    ...
    namespace transports
    {
        namespace ble
        {
            ...
            public:
                ...
                // Central mode methods (scanning and connecting to peripherals)
                ...
                // Peripheral mode methods (advertising and accepting connections)
                virtual bool start_advertising(const std::string &device_name) = 0;
                virtual void stop_advertising() = 0;
                virtual bool register_gatt_service(
                    const std::string &service_uuid,
                    const std::string &char_uuid,
                    std::function<void(const std::string &, const std::vector<uint8_t> &)> on_write,
                    std::function<std::vector<uint8_t>(const std::string &)> on_read) = 0;
                virtual bool notify_characteristic(
                    const std::string &client_address,
                    const std::string &service_uuid,
                    const std::string &char_uuid,
                    const std::vector<uint8_t> &data) = 0;
                virtual std::vector<std::string> get_connected_clients() = 0;
            };
